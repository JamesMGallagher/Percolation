...
public class Percolation {
    ...
    /**
     * Parameter determining the size of the array, which is NxN.
     */
    private final int N;

    /**
     * Weight union find object where each element corresponds to a site in the
     * NxN array and where two elements are in union if and only if there is a
     * path of open sites between the sites they represent.
     */
     ...
     * <dt><b>Precondition:</b><dd> parmN is positive
     */
    ...
    public boolean isFull(final int i, final int j) {
        if (inRange(i, j)) {
            if (isOpen(i, j)) {
                return pathComponents.connected(0, project(i, j));
            } else {
                return false;
            }
        } else {
